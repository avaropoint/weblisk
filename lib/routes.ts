...
import { html, css, js } from "./helpers.ts";
...
export interface WebliskAppConfig {
  ...
  htmlTemplate?: (content: string, data: any, meta: WebliskMeta, htmlConfig: WebliskHtmlConfig) => string;
  globalStyles?: (data: any) => string;
  globalClientCode?: (data: any) => string;
  globalData?: (context: RouteContext) => Promise<any> | any;
  globalEvents?: Record<string, (data: any, context: any) => Promise<any> | any>;
...
export class WebliskRoute {
  ...
  constructor(config: Partial<WebliskFrameworkRouteConfig> = {}) {
    ...
    this.config = {
      ...
      data: async (context) => this.buildData(context),
      ...
      ...Object.fromEntries(Object.entries(this.userConfig).filter(([key]) => key !== 'template'))
    };
   ...
   */
  protected defaultHtmlTemplate(data: any): string {
    ...
    if (WebliskRoute.appConfig.htmlTemplate) {
      const userContent = this.userConfig.template ? this.userConfig.template(data) : '<h1>Welcome to Weblisk</h1>';
      return WebliskRoute.appConfig.htmlTemplate(userContent, data, meta, htmlConfig);
    }

    // Default framework template with comprehensive meta support
    return html`
      <!DOCTYPE html>
      <html lang="${htmlConfig.lang || 'en'}" ${htmlConfig.dir ? `dir="${htmlConfig.dir}"` : ''}>
      <head>
        ...
        <meta name="description" content="${meta.description || data.description || 'Built with Weblisk Framework'}">
        ...
        ${meta.appleTouchIcon ? `<link rel="apple-touch-icon" href="${meta.appleTouchIcon}">` : ''}
        
        ${this.buildOpenGraphMeta(meta.og)}
        ${this.buildTwitterMeta(meta.twitter)}
        ${this.buildCustomMeta(meta.custom)}
        
        <style>${this.buildStyles(data)}</style>
      </head>
      <body ${htmlConfig.bodyClass ? `class="${htmlConfig.bodyClass}"` : ''} ${htmlConfig.bodyId ? `id="${htmlConfig.bodyId}"` : ''}>
        <div ${htmlConfig.appContainerId ? `id="${htmlConfig.appContainerId}"` : 'id="app"'} ${htmlConfig.appContainerClass ? `class="${htmlConfig.appContainerClass}"` : ''}>
            ${this.userConfig.template ? this.userConfig.template(data) : '<h1>Welcome to Weblisk</h1>'}
        </div>
        <script>
  ...
  protected buildStyles(data: any): string {
    const frameworkStyles = css``; // empty
    ...
    const routeStyles = userRouteConfig.styles ? userRouteConfig.styles(data) : '';
   ...
   */
  protected buildClientCode(data: any): string {
    ...
    return [frameworkClientCode, appClientCode, userRouteClientCode].filter(Boolean).join('\n\n');
  }

  /**
   * Build the complete data by merging framework → app → route data
   */
  protected async buildData(context: RouteContext): Promise<any> {
    const frameworkData = {
   ...
   */
  protected buildEvents(): Record<string, any> {
  ...
  protected buildMeta(): WebliskMeta {
    ...
    if (appMeta.custom || userRouteMeta.custom) {
      merged.custom = [...(appMeta.custom || []), ...(userRouteMeta.custom || [])];
    }
   ...
   */
  async handleEvent(eventName: string, data: any, context: any): Promise<any> {
